...
逐节学习入口：[第 25–28 周课程教程](../tutorials/README.md)。

## 本阶段语言定位

第 25–28 周以业务证据、流程和接入设计为主，语言中立。需要技术 Spike 时只复用既有 Java 控制面、Python 执行面或最小 TypeScript 控制台，不为了原型引入新语言；所有设计必须能映射到现有职责边界。完整语言地图见[开发语言路线](../01-language-roadmap.md)。
