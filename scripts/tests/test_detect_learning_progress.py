...
class ProgressScannerTest(unittest.TestCase):
    def run_scan(self, repo: Path) -> dict:
    ...
    def create_repo(
        ...
        *,
        ...
        (root / "learning-plan" / "stages").mkdir(parents=True)
        (root / "deliverables").mkdir()
        (root / "artifacts").mkdir()

        (root / "AGENTS.md").write_text("# Rules\n", encoding="utf-8")
        (root / "README.md").write_text("# Course\n", encoding="utf-8")
        (root / "learning-plan" / "README.md").write_text(
            "# 32 周完整学习路线\n", encoding="utf-8"
        )
        (root / "learning-plan" / "00-learning-method.md").write_text(
            "# 学习方法\n", encoding="utf-8"
        )
        (root / "learning-plan" / "06-deliverable-standards.md").write_text(
            "# 成果标准\n", encoding="utf-8"
        )
        (root / "learning-plan" / "07-instructor-supervision.md").write_text(
            "# 讲师监督\n", encoding="utf-8"
        )
        (root / "learning-plan" / "stages" / "README.md").write_text(
            "# 阶段目录\n\n"
            "| 阶段 | 周次 | 文档 |\n"
            "| --- | --- | --- |\n"
        ...
        (root / "learning-plan" / "stages" / "stage-01.md").write_text(
            "# 阶段一\n\n"
            "## 第 1 周：基础\n\n"
            "| 日期 | 学习任务 | 当天学习建议 | 当天验收 |\n"
            "| --- | --- | --- | --- |\n"
        ...
        (root / "learning-plan" / "stages" / "stage-02.md").write_text(
            "# 阶段二\n", encoding="utf-8"
        )
        ...
        (root / "deliverables" / "README.md").write_text(
            "\n".join(index_lines) + "\n", encoding="utf-8"
        )
        ...
        for week in range(1, 33):
            if week == omit_week:
                continue
            week_dir = root / "deliverables" / f"week-{week:02d}"
            week_dir.mkdir()
            default = (
                f"# Week {week:02d}\n\n"
                "状态：`未开始`\n\n"
    ...
    def passed_week(root: Path, week: int) -> str:
        artifact = root / "artifacts" / f"week-{week:02d}.md"
    ...
    def test_teacher_passed_week_advances_to_next_week(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo / "deliverables" / "week-01" / "README.md").write_text(
    ...
    def test_missing_week_readme_makes_repository_invalid(self) -> None:
        ...
        self.assertIn("deliverables/week-07/README.md", result["missing_required_files"])
    ...
    def test_rules_fingerprint_changes_when_stage_plan_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stage = repo / "learning-plan" / "stages" / "stage-01.md"
            stage.write_text(
